...
export class QueryClientImpl implements Query {
  ...
  Params(request: QueryParamsRequest): Promise<QueryParamsResponse> {
    ...
    const promise = this.rpc.request("yogeshguptakaithal.testchain.testchain.Query", "Params", data);
    return promise.then((data) => QueryParamsResponse.decode(new _m0.Reader(data)));
  }

  Comment(request: QueryGetCommentRequest): Promise<QueryGetCommentResponse> {
    const data = QueryGetCommentRequest.encode(request).finish();
    const promise = this.rpc.request("yogeshguptakaithal.testchain.testchain.Query", "Comment", data);
  ...
  CommentAll(request: QueryAllCommentRequest): Promise<QueryAllCommentResponse> {
    ...
    const promise = this.rpc.request("yogeshguptakaithal.testchain.testchain.Query", "CommentAll", data);
