import { Client, registry, MissingWalletError } from 'yogeshguptakaithal-testchain-client-ts'

import { Comment } from "yogeshguptakaithal-testchain-client-ts/yogeshguptakaithal.testchain.testchain/types"
import { Params } from "yogeshguptakaithal-testchain-client-ts/yogeshguptakaithal.testchain.testchain/types"
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: yogeshguptakaithal.testchain.testchain initialized!')
		...
		async QueryParams({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.YogeshguptakaithalTestchainTestchain.query.queryParams()).data
		...
		async QueryComment({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.YogeshguptakaithalTestchainTestchain.query.queryComment( key.id)).data
		...
		async QueryCommentAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.YogeshguptakaithalTestchainTestchain.query.queryCommentAll(query ?? undefined)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.YogeshguptakaithalTestchainTestchain.query.queryCommentAll({...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
		...
		async sendMsgCreateComment({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.YogeshguptakaithalTestchainTestchain.tx.sendMsgCreateComment({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async sendMsgUpdateComment({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.YogeshguptakaithalTestchainTestchain.tx.sendMsgUpdateComment({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async sendMsgDeleteComment({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.YogeshguptakaithalTestchainTestchain.tx.sendMsgDeleteComment({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async MsgCreateComment({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.YogeshguptakaithalTestchainTestchain.tx.msgCreateComment({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgCreateComment:Init Could not initialize signing client. Wallet is required.')
		...
		async MsgUpdateComment({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.YogeshguptakaithalTestchainTestchain.tx.msgUpdateComment({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgUpdateComment:Init Could not initialize signing client. Wallet is required.')
		...
		async MsgDeleteComment({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.YogeshguptakaithalTestchainTestchain.tx.msgDeleteComment({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgDeleteComment:Init Could not initialize signing client. Wallet is required.')
